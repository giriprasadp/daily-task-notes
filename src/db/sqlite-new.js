...
// CRUD helpers for rich notes (section-based)
export async function getRichNotes(section = 'personal') {
    const db = await getDB()
    const res = db.exec('SELECT markdown FROM rich_notes WHERE section = ?', [section])
    return res.length ? res[0].values[0][0] : ''
}
...
export async function setRichNotes(section = 'personal', markdown) {
    ...
    const existing = db.exec('SELECT id FROM rich_notes WHERE section = ?', [section])

    if (existing.length > 0) {
        // Update existing
        db.run('UPDATE rich_notes SET markdown = ?, updated_at = ? WHERE section = ?', [markdown, now, section])
    } else {
        // Insert new
        db.run('INSERT INTO rich_notes (id, section, markdown, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
            [section, section, markdown, now, now])
    }
...
export async function listRichNotesSections() {
    const db = await getDB()
    const res = db.exec('SELECT section, created_at, updated_at FROM rich_notes ORDER BY updated_at DESC')
    if (res.length === 0) return []
    const rows = res[0]
    return rows.values.map(([section, created_at, updated_at]) => ({
        section,
        created_at,
        updated_at
    }))
...
export async function deleteRichNotesSection(section) {
    const db = await getDB()
    db.run('DELETE FROM rich_notes WHERE section = ?', [section])
    await persistDB()
}
